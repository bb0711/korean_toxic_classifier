...
import time
import datetime
import data_helpers_fast as data_helpers
...
class TextCNN(object):
	...
	def __init__(self, sequence_length, num_classes, word_dict, embedding_matrix,
			...
			for i, filter_size in enumerate(filter_sizes):
				with tf.name_scope("conv-maxpool-%s" % filter_size):
					...
					conv = tf.nn.conv2d(
						...
						# self.embedded_chars_expanded, # input= [batch, height, width, in channel]
		...
		with tf.name_scope("loss"):
			...
			#losses =focal_loss_sigmoid(self.input_y, self.scores, 0.25, 2)
...
tf.flags.DEFINE_string("positive_data_file", "./text/pos_clear_k.txt", "Data source for the positive data.")
tf.flags.DEFINE_string("negative_data_file", "./text/neg_clear_k.txt", "Data source for the negative data.")
...
tf.flags.DEFINE_integer("num_epochs", 3500, "Number of training epochs (default: 200)")
...
def preprocess():
	...
	# Load data
	# print("Loading data...")
	x, y, word_dict, embedding_matrix = data_helpers.load_data_and_labels(FLAGS.positive_data_file,
...
def train(x_train, y_train, word_dict, embedding_matrix, x_dev, y_dev):
	...
	with tf.Graph().as_default():
		...
		with sess.as_default():
			cnn = TextCNN(
				sequence_length=46,  # max_document_length,#x_train.shape[0].shape[1],
			...
			def train_step(x_batch, y_batch):
				...
				_, step, summaries, loss, accuracy, precision, recall, f1 = sess.run(
					[train_op, global_step, train_summary_op, cnn.loss, cnn.accuracy, cnn.precision, cnn.recall, cnn.f1],
					feed_dict)
				time_str = datetime.datetime.now().isoformat()
				if step % 100 == 0:
					print("train --> {}: step {}, loss {:g}, acc {:g}, precision {:g} recall {:g} f1score {:g}".format(time_str, step, loss,
			...
			def dev_step(x_batch, y_batch, final, writer=None):
				...
				time_str = datetime.datetime.now().isoformat()
				if final:
					wf = open('kfast_outpu2.txt', mode='a', encoding='utf-8')
					print("dev final!")
					strtemp = "{}: step {}, loss {:g}, acc {:g}, precision {:g} recall {:g} f1score {:g}\n".format(time_str, step, loss, accuracy,
					                                                                precision, recall, f1)
					wf.write(strtemp)
					print(strtemp)
				if writer:
					print("{}: step {}, loss {:g}, acc {:g} precision {:g} recall {:g} f1score {:g}".format(time_str, step, loss, accuracy,
					                                                               precision, recall, f1))
